# -*- coding: utf-8 -*-
# Downloaded from pdfminer (https://github.com/euske/pdfminer)
# CCITT Fax decoder
#
...
class CCITTFaxDecoder(CCITTG4Parser):
    ...
    def __init__(self, width, bytealign=False, reverse=False):
    ...
    def output_line(self, y, bits):
        bytes_ = array.array('B', [0]*((len(bits)+7)//8))
        if self.reversed:
            bits = [1-b for b in bits]
        for (i, b) in enumerate(bits):
            if b:
                bytes_[i//8] += (128, 64, 32, 16, 8, 4, 2, 1)[i % 8]
        self._buf += bytes_.tostring()
        return
...
def ccittfaxdecode(data, params):
    ...
    reverse = params.get('BlackIs1')
    if K == -1:
        parser = CCITTFaxDecoder(cols, bytealign=bytealign, reverse=reverse)
    else:
        raise ValueError(K)
